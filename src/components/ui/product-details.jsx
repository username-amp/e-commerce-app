...
export const ProductDetails = ({ name, price, ratings, sold }) => {
  const stars = setRatings(ratings || 0);
  ...
  return (
    <div className="flex justify-start items-start flex-col gap-3 flex-wrap">
      <div className="text-lg font-bold tracking-wider max-w-max w-96 ">
        <h1 className="text-lg break-words">{name || "No Product Name"}</h1>
      </div>
      {/* Rates */}
      <section className="flex h-5 items-center space-x-4 text-sm ">
        ...
        <Separator orientation="vertical" className="bg-muted-foreground" />
        <div>
          {sold} <span className="text-sm text-muted-foreground">Sold</span>
        </div>
      </section>

      {/* Price */}
      <section>
        <h2 className="font-bold text-3xl text-[#2B8FD8]">₱{price}</h2>
      </section>
      ...
      <section className="flex flex-row w-full gap-3 justify-between mt-2">
        <Link href={"#"}>
          <Button
            className="h-14 w-56 font-semibold border border-[#2B8FD8] text-[#2B8FD8] hover:text-[#37A6D8] hover:border-[#37A6D8]"
            variant="outline"
          >
            Add To Cart
          </Button>
        ...
        <Link href={"#"}>
          <Button
            className="h-14 w-56 hover:text-muted-foreground font-semibold"
            variant="outline"
          >
            Buy Now
          </Button>
        ...
        <div className="flex flex-col gap-8 ">
          ...
          <div className="flex flex-col gap-5 w-72 justify-start">
            ...
            <div className="flex flex-row gap-5 text-sm w-full justify-between">
              ...
              <span>{!hasWarranty ? "No Warranty" : warrantyDuration}</span>
            </div>
            ...
            <div className="flex flex-row gap-5 text-sm w-full justify-between">
              ...
              <span>
                <span>
                  {!hasWarranty || !warrantyType ? "No Warranty" : warrantyType}
                </span>
              </span>
            </div>
    ...
    <div className="w-full flex justify-center ">
      <section className="flex flex-col  w-2/3 gap-10">
        ...
        {testimonials.map((testimonial, index) => {
          return (
            <div
              className="w-full border border-black rounded-sm p-3"
              key={index}
            >
